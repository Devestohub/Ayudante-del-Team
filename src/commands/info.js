  ...
  execute(message, embed, { client, args, API, prefixUsed, version }) {
    ...
    if (!args.length) {
      const duration = moment
        ...
        .format(
          `y __, M __, w __, d __, h __, m __ [${API.langs.__(
            'words.and'
          )}] s __`
        );
      message.channel.send(
        embed
          .setColor('#7289da')
          .setTitle(
            ':information_source: ' + API.langs.__('commands.info.embed.title')
          )
          ...
          .addFields(
            {
              name:
                ...
                API.langs.__('commands.info.embed.fields.versions.name'),
              value: `✭ ${API.langs.__(
                'commands.info.embed.fields.versions.value',
                {
                  botVersion: `*__v${
            ...
            },
            {
              name:
                ...
                API.langs.__('users.roles.Developer.one'),
              value:
                '<@324449297951096834>\nHugovidafe#2728\n`<@324449297951096834>`',
              inline: true,
            },
            {
              ...
              value:
                '**- ' +
                API.langs.__('commands.info.embed.fields.systemStatistics') +
                '**',
              inline: false,
            },
            {
            ...
            {
              name: API.langs.__('commands.info.embed.fields.running.name'),
              value: duration,
              inline: true,
            }
          )
